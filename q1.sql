SELECT 
citedpaper.title as cited_paper, citerpaper.title as citing_title, citerpaper.year, citerpaper.abstract, authlisting.auth_list, venuelisting.venue  
...
) AS authlisting ON citerpaper.paper_id = authlisting.paperwritten
LEFT JOIN (
    SELECT PublishedIn.paper_id as pubpaper, string_agg(PublishedIn.venueName, ' ') as venue
    FROM PublishedIn
    GROUP BY PublishedIn.paper_id
) AS venuelisting ON citerpaper.paper_id = venuelisting.pubpaper
